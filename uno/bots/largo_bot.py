...
class LargoBot(Player):
    ...
    def choose_action(self) -> PlayerAction:
        ...
        if valid_selections:
            ...
            if wilds:
                ...
                dominant_color = max(
                    color_groups.keys(),
                    key=lambda color: len(color_groups[color])
                )
                dominant_cards = color_groups[dominant_color]
                best_card = max(
                    dominant_cards,
                    key=lambda card: card.label.value
                )
            return self.play_card(best_card)
        else:
            return PlayerAction(draw_card=True)

    def choose_color(self, wildcard: Card) -> CardColor:
        color_counts = {
            color: 0
            for color in [
                CardColor.RED,
    ...
    def should_play_drawn_card(self, drawncard: Card) -> bool:
